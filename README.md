...
OpenTelemetry spans already carry the opaque `enduser.id` FastMCP derives from the OAuth token. To also record the authenticated user's email as the OpenTelemetry `user.email` attribute — so traces can be attributed to a person — opt in explicitly with `export WORKSPACE_MCP_OTEL_USER_EMAIL="true"`. This is PII and is off by default (the variable is unset).
